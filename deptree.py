...
def print_dependencies_recursivelly(binary, full_path, format):
    ...
    for dependency in deps:
        if full_path == True:
            print(format.format(binary = binary, dependency = dependency))
        else:
            bin_basename = os.path.basename(binary)
            dep_basename = os.path.basename(dependency)
            print(format.format(binary = bin_basename, dependency = dep_basename))

        if dependency in visited_dependencies:
            return
        print_dependencies_recursivelly(dependency, full_path, format)
